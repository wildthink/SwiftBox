...
func topics (text: String) -> [TaggedToken] {
    return tag(text, scheme: NSLinguisticTagSchemeNameTypeOrLexicalClass)
}

topics ("Jon Smith took a plane to New York City. The plane was fast.")
...
public class NaiveBayesClassifier {
    ...
    private func totalOccurrencesOfToken(token: String) -> Int {
        if let occurrences = tokenOccurrences[token] {
            return occurrences.values.reduce(0, combine: +)
        }
        return 0
    }
